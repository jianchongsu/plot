...
#### 第一步：生成数据
```{r data,warning=FALSE,tidy=TRUE}
...
#### 第二步 ：设置前景色和作图区域
```{r par,warning=FALSE,tidy=TRUE,fig.height=6,fig.width=6}
plot(x.value,y.value,xlab = NA,ylab = NA,ylim = c(-6,8),xlim = c(-10,10),type = "l",
     lwd = 10,col = rainbow(8)[7],xaxt="n",yaxt = "n")
...
```{r rain.bow,warning=FALSE,tidy=TRUE}
rain.bow=function(r,n)
{
  curve(sqrt((r)^2-x^2)-2,from=-r,to=r,type="l",lwd=10,col=rainbow(8)[n],add=TRUE)
}
```
那么余下的工作就可以如下完成了（其实这个也可以程序化）：

```{r rain.bow.output,warning=FALSE,tidy=TRUE,fig.height=6,fig.width=8}
plot(x.value,y.value,xlab = NA,ylab = NA,ylim = c(-6,8),xlim = c(-10,10),type = "l",
     lwd = 10,col = rainbow(8)[7],xaxt="n",yaxt = "n")
rain.bow(4.4,6)
rain.bow(4.8,5)
...
```{r rainbow(7).bar,warning=FALSE,echo=FALSE,fig.height=8,fig.width=8}
barplot(1:7,col=rainbow(7))
```
这七种颜色中缺少橙色，却多了粉红色。
于是需要将之前的少许颜色略作更改即可：
```{r finish,echo=FALSE,warning=FALSE,fig.height=6,fig.width=8}
par(bg="skyblue3")
x.value=seq(-4,4,length=1000)
y.value=sqrt(16-x.value^2)-2
plot(x.value,y.value,xlab = NA,ylab = NA,ylim = c(-6,8),xlim = c(-10,10),type = "l",
...
curve(sqrt((5.2)^2-x^2)-2,from=-5.2,to=5.2,type="l",lwd=10,col="green",add=TRUE)
curve(sqrt((5.6)^2-x^2)-2,from=-5.6,to=5.6,type="l",lwd=10,col="yellow",add=TRUE)

curve(sqrt((6)^2-x^2)-2,from=-6,to=6,type="l",lwd=10,col=rainbow(8)[2],add=TRUE)
curve(sqrt((6.4)^2-x^2)-2,from=-6.4,to=6.4,type="l",lwd=10,col="red",add=TRUE)
